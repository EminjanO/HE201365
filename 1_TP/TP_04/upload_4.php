...
if(isset($_FILES["fichierUpload"]))
{
    ...
    if(in_array($extentionUpload, $extentinsValides))
    {
        ...
        if(move_uploaded_file($_FILES['fichierUpload']['tmp_name'], $dossier . $fichier))
        {
            ...
            // on crée une ressource representant
            ...
            if($size[0] > $size[1])
            {
                // creer un resource pour miniature
                $im = imagecreate(round(($avatarMaxSize/$size[1])*$size[0]), $avatarMaxSize);
                // placer dand la ressource que nous venons de créer une copie de l'image originalle,
            ...
            else
            {
                ...
                // placer dans la ressource que nous venons de créer une copie de l'image originalle,
                // redimensionnée et réechantillonée
                imagecopyresampled($im, $src, 0,0,0,0, $avatarMaxSize, round(($avatarMaxSize/$size[0])*$size[1]),
                    $size[0], size[1]);
            }
