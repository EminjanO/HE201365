...
# 3) Afficher la superfie totale du "BENELUX"
SELECT
...
# 2). Afficher les "filles" d'une "section" donnée (p.e TI)
...
#Dans "world" : multi table
# pour le "monde", donnez le pourcentage de personnes qui ne parlent pas une langue officielle

# Comprenez : personnes répertoriées comme ne parlant pas une langue officielle
# ou : répertoriées comme parlant une langue non-officielle
...
# 2). Afficher les "filles" d'une facultée donnée (p.e TI)
...
SELECT 
    ...
    user.nom,
    user.prenom,
    CONCAT(LCASE(user.username), '@students.ephec.be') AS 'email'
FROM
    minicampus.user
      left  JOIN
...
# 2s)  Pour chacune des "facultés", afficher ces informations précédées du nom de son "parent"
